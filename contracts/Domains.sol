...
// This is a Solidity contract to setup a domain name service. it is similar to the ethereum name service that ends in '.eth' however, this one will be doployed on the polygon network and will be ending with whatever is passed in the tld when deploying the contract.
...
contract Domains is ERC721URIStorage {
    ...
    // these are gotten from the imports up there. they are used to track pur token ids
    ...
    //  another mapping data type to map records and domain names
    mapping(string => string) public records;

    // our main TLD
    string public tld;

    // svg icon code for the nft we're making for the domain. 
  ...
  string svgPartTwo = '</text></svg>';
    ...
    constructor(string memory _tld) payable ERC721("lima name service", "LNS") {
        tld = _tld;
        console.log("%s nameservice deployed", tld);
    }

    // register function that adds a new name to the mapping
    function register(string calldata name) public payable {

        // making sure the name is not already registered
    ...
    _tokenIds.increment();
        ...
        console.log("person %s has registered a domain", _name);
    }

    // get the address of the owner of a name
    function getAddress(string calldata _name) public view returns (address) {
        return domains[_name];
    }

    // set reords of the owner of a name
